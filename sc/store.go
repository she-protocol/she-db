...
import (
	...
	"github.com/she-protocol/she-db/sc/memiavl"
	"github.com/she-protocol/she-db/sc/types"
)
...
func (cs *CommitStore) LoadVersion(targetVersion int64, copyExisting bool) (types.Committer, error) {
	cs.logger.Info(fmt.Sprintf("SheDB load target memIAVL version %d, copyExisting = %v\n", targetVersion, copyExisting))
	if copyExisting {
		opts := cs.opts
